...
export const env = createEnv({
  ...
  experimental__runtimeEnv: {
    ...
    NEXT_PUBLIC_DHARAHIL_ENABLED: process.env.NEXT_PUBLIC_DHARAHIL_ENABLED,

    // Server-only vars (TASKPILOT_*) are deliberately absent:
    // experimental__runtimeEnv takes client vars only, because those are what
    // Next inlines at build. Server vars are read from process.env at runtime.
  },
});
