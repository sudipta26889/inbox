...
description: 'Reference for all environment variables used in Inbox'
---

Comprehensive reference for all environment variables relevant to self-hosting Inbox.
...
| `RESEND_FROM_EMAIL` | No | From email address | `Inbox <updates@transactional.inbox.sudiptadhara.in>` |
...
| `NEXT_PUBLIC_BRAND_NAME` | No | Brand name used in UI text and metadata | `Inbox` |
| `NEXT_PUBLIC_BRAND_LOGO_URL` | No | Custom logo URL or public asset path (for example `/images/brand-logo.svg`) | Built-in Inbox logo |
| `NEXT_PUBLIC_BRAND_ICON_URL` | No | Custom app icon URL or public asset path | `/icon.png` |
| `NEXT_PUBLIC_SUPPORT_EMAIL` | No | Contact email shown in support links and error messages | `elie@inbox.sudiptadhara.in` |
| **Debugging** ||||
