...
Reply Zero labels every email that needs a reply as `To Reply`, and every email where you're waiting for a reply as `Awaiting Reply`. These labels appear in your regular email client (Gmail or Outlook). Gmail users can also use the Reply Zero view in Inbox to see only these emails.
...
Go to the [Reply Zero](https://www.inbox.sudiptadhara.in/reply-zero) tab in the left sidebar to enable it.
