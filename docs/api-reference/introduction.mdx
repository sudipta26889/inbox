...
Use the Inbox API to manage emails and automation rules programmatically.

If you prefer a CLI wrapper for scripts or bots, see the [API CLI](/api-reference/cli).

## Supported Email Providers

Inbox supports integration with:
- **Gmail** (Google Workspace and personal accounts)
- **Outlook** (Microsoft 365 and personal accounts)

## Getting Started

To begin using the Inbox API, you'll need to obtain an API key. Here's how:

1. Log in to your Inbox account
2. Navigate to the [Settings](https://www.inbox.sudiptadhara.in/settings) page and scroll down to the `API Keys` section
...
If you are self-hosting Inbox, set the `NEXT_PUBLIC_EXTERNAL_API_ENABLED=true` environment variable and rebuild the app to enable the API. Use your deployment URL as the base URL for requests (for example, `https://your-domain.com/api/v1`).
