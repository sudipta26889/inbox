...
async function main() {
  ...
  program
    .name("inbox-zero")
    .description(
      "CLI tool for self-hosting Inbox — AI email assistant.\n\n" +
        "Quick start:\n" +
        "  inbox-zero setup      Configure OAuth providers, AI provider, and Docker\n" +
        "  inbox-zero start      Start Inbox\n" +
        "  inbox-zero config     View and update settings\n\n" +
        "Docs: https://docs.inbox.sudiptadhara.in/self-hosting",
    )
  ...
  program
    .command("start")
    .description("Start Inbox")
    .option("--no-detach", "Run in foreground (default: background)")
    .action(runStart);

  program.command("stop").description("Stop Inbox").action(runStop);
  ...
  program
    .command("setup-aws")
    .description("Deploy Inbox to AWS using Copilot (ECS/Fargate)")
...
async function runSetupQuick(options: { name?: string }) {
  ...
  if (wantsGoogle) {
    ...
    p.note(
      "You need a Google OAuth app to connect your Gmail.\n\n" +
        ...
        "Full guide: https://docs.inbox.sudiptadhara.in/hosting/setup-guides",
      "Google OAuth",
    );
  ...
  if (wantsMicrosoft) {
    ...
    p.note(
      "You need a Microsoft app registration to connect Outlook.\n\n" +
        ...
        "Full guide: https://docs.inbox.sudiptadhara.in/hosting/setup-guides#microsoft-oauth-setup",
      "Microsoft OAuth",
    );
  ...
  if (wantsGoogle) {
    p.note(
      "Google Pub/Sub enables real-time email notifications.\n\n" +
        ...
        "Full guide: https://docs.inbox.sudiptadhara.in/hosting/setup-guides#google-pubsub-setup",
  ...
  if (checkContainersRunning(composeArgs)) {
    const restart = await p.confirm({
      message: "Inbox is already running. Restart?",
      initialValue: true,
    });
    if (p.isCancel(restart) || !restart) {
      p.note(
        `Inbox is still running at http://localhost:${webPort}`,
        "Already running",
      );
      p.outro("Setup complete!");
      return;
  ...
  startSpinner.start("Starting Inbox...");
  ...
  startSpinner.stop("Inbox is running!");
...
async function runSetupAdvanced(options: { name?: string }) {
  const configName = options.name;
  p.intro(`🚀 Inbox Setup${configName ? ` (${configName})` : ""}`);
...
Full guide: https://docs.inbox.sudiptadhara.in/self-hosting/google-oauth`,
      "Google OAuth Setup",
    );
...
Full guide: https://docs.inbox.sudiptadhara.in/self-hosting/google-pubsub`,
...
Full guide: https://docs.inbox.sudiptadhara.in/self-hosting/microsoft-oauth`,
...
async function runStart(options: { detach: boolean }) {
  ...
  if (!existsSync(STANDALONE_COMPOSE_FILE)) {
    p.log.error(
      "Inbox is not configured for production mode.\n" +
        "Run 'inbox-zero setup' and choose Production (Docker) first.",
    );
    process.exit(1);
  }

  p.intro("🚀 Starting Inbox");
  ...
  if (checkContainersRunning(composeArgs)) {
    ...
    if (p.isCancel(restart) || !restart) {
      p.outro("Inbox is already running.");
      return;
    }
    const stopSpinner = p.spinner();
    stopSpinner.start("Stopping existing containers...");
...
async function runStop() {
  ...
  if (!existsSync(STANDALONE_COMPOSE_FILE)) {
    p.log.error("Inbox is not configured.");
    process.exit(1);
  }

  p.intro("Stopping Inbox");
